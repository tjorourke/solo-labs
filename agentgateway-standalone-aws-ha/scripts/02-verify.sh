...
for id in $(fleet_instances); do
  ver="$(node_try "$id" '/usr/local/bin/agentgateway --version | jq -r .version' | tr -d '\n ')"
  sum="$(node_try "$id" 'sha256sum /etc/agentgateway/config.yaml | cut -c1-12' | tr -d '\n ')"
...
for id in $(fleet_instances); do
  states="$(node_try "$id" 'for u in agentgateway agw-echo agw-ratelimit; do printf "%s=%s " $u $(systemctl is-active $u); done; printf "config-sync-timer=%s" $(systemctl is-active agw-config-sync.timer)' 2>/dev/null)"
  printf '  %-20s %s\n' "$id" "$states"
done
...
# Note: the gateway answers oauth-protected-resource for any path suffix once
# mcpAuthentication is configured, so a 200 there does not prove a route exists.
# Check the body's resource field instead.
expect "resource metadata names the MCP endpoint" "$GATEWAY_URL/mcp" \
  "$(curl -s "$GATEWAY_URL/.well-known/oauth-protected-resource/mcp" | jq -r '.resource // ""')"
...
rows="$(node_try "$one_node" "set -a; . /etc/agentgateway/env; set +a; psql \"\$AGW_DATABASE_URL\" -Atc \"select count(*) from information_schema.tables where table_schema='public'\"" 2>/dev/null | tr -d '\n ')"
