#!/usr/bin/env bash
# ask.sh <agent> "<prompt>" — call one of the agents over A2A and print its reply.
...
AGENT="${1:-dba-diagnoser}"; shift || true
PROMPT="${*:-the orders database is down - take a look}"
PROMPT="$(printf '%s' "$PROMPT" | tr '\n' ' ' | sed 's/  */ /g')"

step "Calling $AGENT"
kc -n kagent port-forward svc/kagent-controller 18083:8083 >/tmp/a2a-pf.$$ 2>&1 & CPF=$!
trap 'kill $CPF 2>/dev/null' EXIT
for _ in $(seq 1 20); do curl -s -o /dev/null "http://localhost:18083/api/a2a/kagent/${AGENT}/.well-known/agent.json" && break; sleep 1; done

RESP="$(curl -s -X POST "http://localhost:18083/api/a2a/kagent/${AGENT}/" \
