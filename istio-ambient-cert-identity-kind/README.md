...
An ambient-only kind lab (Solo Enterprise for Istio, ambient mode) that walks a petshop app through the whole ambient security model: SPIFFE identity, L4 authorization in ztunnel, identity-aware access logs, an opt-in waypoint, a Keycloak IdP, claim-based L7 JWT authorization, and the Solo Enterprise differences — ending with **workload claims** closing the shared-ServiceAccount gap, live.

- **Edition:** Enterprise (Solo Istio Helm charts + Solo images).
- **Validated live on:** Solo Istio `1.30.3-solo` (charts and images — on the 1.30 line the image tag keeps `-solo`; the plain `1.30.3` tag is the upstream build), Gateway API `v1.5.1`.
