...
An ambient-only kind lab (Solo Enterprise for Istio, ambient mode) that walks a petshop app through the whole ambient security model: SPIFFE identity, L4 authorisation in ztunnel, identity-aware access logs, an opt-in **agentgateway waypoint** (JWT + CEL authorisation, canary routing, rate limiting by workload identity), a Keycloak IdP, and the Solo Enterprise differences: ending with **workload claims** closing the shared-ServiceAccount gap, live.
...
<a id="the-cast-namespace-petshop-ambient"></a>
## Workloads and ServiceAccounts (namespace `petshop`, ambient)
...
<a id="closing-the-gap--workload-claims"></a>
