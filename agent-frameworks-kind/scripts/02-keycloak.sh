...
# (users alice/bob/carol; alice in group field-fte; public client `kagent` with
# audience + groups mappers). This is the IdP whose tokens kagent exchanges.
...
kc -n "$KEYCLOAK_NS" rollout status statefulset/keycloak --timeout=300s >/dev/null \
  || warn "keycloak not Ready in 5m — check: kubectl -n $KEYCLOAK_NS get pods"
ok "Keycloak up; issuer ${KEYCLOAK_ISSUER}"

step "Keycloak ready"; echo "  Next: ./scripts/03-agentgateway.sh" >&2
