#!/usr/bin/env bash
# lib.sh — shared helpers for agentic-a2a-kind (Keycloak + kagent-enterprise
# OBO + enterprise agentgateway + the A2A SRE/DBA agents).
...
export CLUSTER_NAME="${CLUSTER_NAME:-a2a-obo}"
...
# ── secrets ───────────────────────────────────────────────────────────────────
# Required: ANTHROPIC_API_KEY (agent model), plus enterprise license(s):
