...
`demo-console/` is the laptop site for a non-technical audience. Homepage has two cards: live **Gateway decisions** from the task-routing EKS cluster, and **GitHub MCP** (Standard vs Code mode, tokens and $). Spend, budgets and graphs stay in the agentgateway UI. Stop `scripts/30-dashboard.py` first if it still owns 8900, then:
