#!/usr/bin/env bash
# 01-cluster.sh — create the kind cluster + MetalLB + Gateway API CRDs.
#
# Idempotent: skips cluster creation if it already exists; helm/kubectl apply
# patterns are re-runnable. Identical in shape to the agentic-mcp-rbac-kind
# equivalent — only the cluster name (curation) differs.
...
# Derive the kind docker network CIDR and configure MetalLB to hand out a small
# slice of it as LoadBalancer IPs.
