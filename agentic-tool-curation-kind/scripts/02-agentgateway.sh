...
#
# Enterprise (not OSS) because mcp.authorization with CEL on jwt.* and
# mcp.tool.name lives on EnterpriseAgentgatewayPolicy (the enterprise wrapper
# CRD). The OSS AgentgatewayPolicy schema has no equivalent.
#
...
# licenseKey (passed via --set licensing.licenseKey=...).
...
# We now match the proven sibling labs' pattern (release names without the
# `enterprise-` prefix). If those old releases exist, their CRD ownership
# annotations block upgrade-install under the new name. Uninstalling drops the
# helm release but keeps the CRDs in-place; the next install adopts them.
...
echo "  Next: ./scripts/03-agentregistry.sh" >&2
