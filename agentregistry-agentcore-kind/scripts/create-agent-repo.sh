...
# Copy the lab folder (the folder .gitignore drops .env.local / .agentcore / venv).
cp -R "$LAB_ROOT"/* "$TMP"/ 2>/dev/null || true
cp "$LAB_ROOT/.gitignore" "$TMP"/ 2>/dev/null || true
rm -rf "$TMP/.agentcore" "$TMP/.env.local"
git -C "$TMP" add -A
