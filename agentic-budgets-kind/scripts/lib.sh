#!/usr/bin/env bash
# lib.sh — shared helpers for agentic-budgets-kind scripts.
#
# Sourced by every script under ./scripts/. Mirrors the agentic-hitl-kind and
# agentic-mcp-rbac-kind labs so the trio share muscle memory.
...
# ── cluster constants ─────────────────────────────────────────────────────────
export CLUSTER_NAME="${CLUSTER_NAME:-budgets}"
export CTX="kind-${CLUSTER_NAME}"

# Enterprise agentgateway chart + version. This lab needs the enterprise data
# plane because token-based rate limiting + RateLimitConfig +
# EnterpriseAgentgatewayPolicy.traffic.entRateLimit are enterprise-only
# (the OSS chart at cr.agentgateway.dev does not ship the
# ratelimit.solo.io API group).
#
...
# kube-prometheus-stack chart — pinned to a recent stable version.
export KPS_CHART="${KPS_CHART:-prometheus-community/kube-prometheus-stack}"
...
#   AGENTGATEWAY_LICENSE_KEY — Solo Enterprise AGW license; data plane refuses
#                              to start without it
#   ANTHROPIC_API_KEY        — kagent default model (the BYO chat agents in
#                              this lab don't depend on it; they call the
#                              mock LLM via the gateway; but kagent's
#                              dashboard features want it)
...
# two Secrets (jwt-dba, jwt-support) from inside the pod.
wait_secret() {
