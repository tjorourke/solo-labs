...
# keep it, run the router, send everything to the decision gateway. Then puts the intake
# gateway in front, which rewrites the model name to the one the router answers to and drops
# tool shapes the backends cannot take, so an editor or an agent works without being told
# what to send. scripts/99-restore.sh puts Part 3's back.
...
banner "the intake gateway in front"
kubectl apply -f "$HERE/yaml/90-intake-gateway.yaml"
kubectl -n "$NS" wait --for=condition=Programmed gateway/intake-gateway --timeout=180s
