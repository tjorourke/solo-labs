#!/usr/bin/env bash
# quick.sh — one-shot orchestrator for agentic-pii-guardrail-kind.
#
# Usage:
...
case "$cmd" in
  up)
    ...
    bash "$SCRIPT_DIR/03-guardrail-and-ui.sh"
    bash "$SCRIPT_DIR/04-policy.sh"

    echo ""
    cat >&2 <<EOF
══════════════════════════════════════════════════════════════════
  agentic-pii-guardrail-kind — UP
══════════════════════════════════════════════════════════════════
...
EOF
  ...
  status)
    ...
    for ns in agentgateway-system pii-demo; do
      echo "  ─ $ns" >&2
