#!/usr/bin/env bash
# lib.sh — shared helpers for agentregistry-arctl-kind.
#
# Story: scaffold + build an MCP server, a skill, and an agent with arctl, wire
# them together, then host the agent on Solo Enterprise for kagent in a kind
# cluster. The OSS arctl daemon (localhost:12121) is the catalog/control plane;
# a Kubernetes Runtime points it at the kind cluster so a Deployment lands as
# kagent CRDs. Anthropic is the model provider.
