#!/usr/bin/env bash
# quick.sh — one-shot orchestrator for the agentic-mcp-rbac-kind lab.
#
# Usage:
#   ./quick.sh up        — run 01..06 in order (idempotent on rerun)
...
case "$cmd" in
  up)
    ...
    bash "$SCRIPT_DIR/05-rbac-policy.sh"

    echo ""
    cat >&2 <<EOF
══════════════════════════════════════════════════════════════════
  agentic-mcp-rbac-kind — UP
══════════════════════════════════════════════════════════════════

  Context:  $CTX
  Next:     ./scripts/port-forward.sh
            then open http://localhost:8090 — the RBAC Inspector UI.
            Use the "Acting as" dropdown to switch between
            alice / bob / carol and watch the tool list change.
...
EOF
  ...
  status)
    ...
    for ns in agentgateway-system kagent ops-tools mcp-rbac; do
      echo "  ─ $ns" >&2
    ...
    step "Inspector UI"
    kc -n mcp-rbac get deploy rbac-inspector-ui 2>/dev/null | sed 's/^/  /' >&2 || true
    ;;
