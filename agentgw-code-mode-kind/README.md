...
result passing through the model. Code mode submits a program through a single
MCP tool call: the filtering and shaping happen in the gateway sandbox,
...
Standalone cluster. Needs an agentgateway licence, plus an Anthropic key for the
Claude step:
