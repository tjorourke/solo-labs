...
# The key the STS signs with. Fleet-wide on purpose: three nodes mint tokens
# independently, and a downstream validator has to accept all three, so they sign
# with one key and publish one JWKS. Give each node its own key and a token minted
# on node A is rejected by a validator that fetched node B's JWKS.
#
# Terraform generates it so the lab needs no openssl step, and it never leaves
# Secrets Manager: the node renders it to /etc/agentgateway/sts-jwt.pem at boot.
resource "tls_private_key" "sts" {
  algorithm = "RSA"
  rsa_bits  = 2048
}
...
resource "aws_secretsmanager_secret_version" "runtime" {
  ...
  secret_string = jsonencode({
    ...
    # The licence. Solo Enterprise for agentgateway checks it at every start and
    # refuses to run without one: "license refused startup: no license key
    # configured, and this installation has never held a valid license; there is
    # no grace period without a prior entitlement". It is read by both processes.
    ENTERPRISE_AGENTGATEWAY_LICENSE_KEY = var.agentgateway_license_key

    # The STS. The proxy reaches it on loopback, so the host never changes; the
    # issuer is published, so it has to be a URL a downstream validator can
    # resolve, which is the ALB in front of the fleet rather than the node.
    STS_HOST        = "127.0.0.1:${local.sts_port}"
    STS_ISSUER      = "${local.gateway_url}/sts"
    STS_SIGNING_KEY = tls_private_key.sts.private_key_pem
