...
variable "name" {
  description = <<-EOT
    Name prefix for every resource in the lab. Distinct from part 1's prefix on purpose,
    so the OSS fleet and the enterprise fleet can exist in one account at the same time
    and a teardown of one cannot take the other with it.
  EOT
  type        = string
  default     = "agw-ent"
}
...
variable "hostname" {
  description = <<-EOT
    Record name created in the zone. The gateway is served at <hostname>.<zone>. Distinct
    from part 1's record, so both fleets can be up at once in the same zone.
  EOT
  type        = string
  default     = "agw-ent"
}
...
variable "agentgateway_version" {
  description = <<-EOT
    Solo Enterprise for agentgateway release to install, with the leading v. Pinned
    rather than latest so a node the Auto Scaling group replaces runs the same build as
    its siblings. The enterprise installer takes this as AGENTGATEWAY_VERSION and
    installs both binaries, the proxy and the STS, at that tag.
  EOT
  type        = string
  default     = "v2026.9.0"
}
...
variable "agentgateway_license_key" {
  description = <<-EOT
    Solo Enterprise for agentgateway licence key. The proxy checks it at every start and
    refuses to run without one, so the fleet does not come up without this. Held in
    Secrets Manager and rendered into /etc/agentgateway/env on each node, never into the
    config file or the AMI. Export AGENTGATEWAY_LICENSE_KEY and the wrapper passes it in.
  EOT
  type        = string
  sensitive   = true
}
