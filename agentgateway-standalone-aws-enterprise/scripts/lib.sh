...
# ---------------------------------------------------------------------------
# A minimal MCP client over streamable HTTP. Shared, because four scripts need it.
# ---------------------------------------------------------------------------

mcp_init() { # mcp_init <token> [base-url] -> prints the session id
  local tok="$1" base="${2:-$GATEWAY_URL}" hdrs
  hdrs="$(mktemp)"
  curl -s -D "$hdrs" -X POST "$base/mcp" \
...
tool_names() { # tool_names <token> [session]
  local tok="$1" sid="${2:-}"
  [[ -n "$sid" ]] || sid="$(mcp_init "$tok")"
...
# Fleet config pushes.
#
# S3 is the source of truth and each node re-syncs every 30 seconds, so a push is
# a copy and a wait. agentgateway watches the file and reloads the dynamic
# sections itself, which is why nothing here restarts anything.
# ---------------------------------------------------------------------------

config_bucket() { tf_out config_bucket; }

# push_config <local-file>  -- copy to S3 and wait for every node to have it
push_config() {
  local src="$1" want bucket
  bucket="$(config_bucket)"
  want="$(sha256sum "$src" | cut -c1-12)"
  ...
  for i in $(seq 1 24); do
    ...
    for id in $ids; do
      local have
      have="$(node_try "$id" 'sha256sum /etc/agentgateway/config.yaml | cut -c1-12' | tr -d ' \n')"
      [[ "$have" == "$want" ]] || all=0
    done
...
# script, which walks the same file through four values.
push_config_field() { # push_config_field <yaml-path-key> <value>
  local key="$1" val="$2" tmp
  tmp="$(mktemp)"
  sed -E "s/^(  $key: ).*/\1$val/" "$LAB_DIR/config/config.yaml" > "$tmp"
  grep -qE "^  $key: $val\$" "$tmp" || die "could not set $key to $val in the config"
  push_config "$tmp"
  rm -f "$tmp"
}
