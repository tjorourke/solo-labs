...
if [[ -z "${AGENTGATEWAY_LICENSE_KEY:-}" ]]; then
  warn "set AGENTGATEWAY_LICENSE_KEY=<your Solo Enterprise for agentgateway key>"
  warn "The proxy checks the key at every start and refuses to run without one, so the"
  warn "fleet would build and then never go healthy:"
  warn "  license refused startup: no license key configured, and this installation has"
  warn "  never held a valid license; there is no grace period without a prior entitlement"
  fails=$((fails+1))
else
  ok "AGENTGATEWAY_LICENSE_KEY is set (${#AGENTGATEWAY_LICENSE_KEY} chars)"
  log "It is stored in Secrets Manager by the Terraform and rendered into"
  log "/etc/agentgateway/env on each node. It never lands in the config file or in git."
fi
