...
case "${1:-up}" in
  ...
  test)
    ...
    ;;
  features)
    for s in 10-routing 11-auth 12-llm 13-mcp 15-ratelimit \
             16-tool-modes 17-composable 18-token-exchange; do
      hdr "scripts/$s.sh"; "$S/$s.sh"
    done
    ;;
  ha)
    for s in 20-ha-node-loss 21-ha-mcp-session 22-ha-config-push 23-ha-ui-overlay \
             24-ha-sts-loss; do
      hdr "scripts/$s.sh"; "$S/$s.sh"
    done
    ;;
