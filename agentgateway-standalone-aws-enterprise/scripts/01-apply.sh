...
# Solo Enterprise refuses to start without a licence key, so the build is stopped
# here rather than 15 minutes later with three nodes that will never go healthy.
[[ -n "${AGENTGATEWAY_LICENSE_KEY:-}" ]] \
  || die "set AGENTGATEWAY_LICENSE_KEY=<your Solo Enterprise for agentgateway licence key>" 
...
tf plan -input=false -out=tfplan \
  ...
  -var "agentgateway_license_key=$AGENTGATEWAY_LICENSE_KEY" \
...
cat <<EOT
  ...
  scripts/16-tool-modes.sh      standard, search, code and codeSearch measured
  scripts/17-composable.sh      one tool name, several HTTP steps, one answer
  scripts/18-token-exchange.sh  impersonation, delegation, dual authentication
  ...
  scripts/24-ha-sts-loss.sh     kill the STS and watch the node leave the pool
